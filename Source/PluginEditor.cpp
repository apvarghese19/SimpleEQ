...
SimpleEQAudioProcessorEditor::SimpleEQAudioProcessorEditor (SimpleEQAudioProcessor& p)
    ...
    lowCutFreqSliderAttachment(audioProcessor.apvts, "low_cut_freq", lowCutFreqSlider),
    highCutFreqSliderAttachment(audioProcessor.apvts, "high_cut_freq", highCutFreqSlider),
    lowCutSlopeSliderAttachment(audioProcessor.apvts, "low_cut_slope", lowCutSlopeSlider),
    highCutSlopeSliderAttachment(audioProcessor.apvts, "high_cut_slope", highCutSlopeSlider)
{
...
void SimpleEQAudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    g.setFont(Font(FontOptions("Iosevka Term Slab", 30, 0)));
    
    String title { "AV Simple EQ" };
    g.setFont(30);
    ...
    {
        double mag = 1.f;
        // This ensures 'freq' NEVER hits the Nyquist limit, even if sampleRate is low.
        auto freq = mapToLog10(double(i) / double(width), 20.0, std::min(20000.0, sampleRate * 0.45));
...
void SimpleEQAudioProcessorEditor::timerCallback()
{
    if( parametersChanged.compareAndSetBool(false, true))
    {
        ...
        auto highCutCoeffiecients = makeHighCutFilter(chainSettings, audioProcessor.getSampleRate());
        
        updateCutFilter(monoChain.get<ChainPositions::LowCut>(), lowCutCoeffiecients, chainSettings.lowCutSlope);
        updateCutFilter(monoChain.get<ChainPositions::HighCut>(), lowCutCoeffiecients, chainSettings.highCutSlope);
